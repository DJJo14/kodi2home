...
class kodi2home():
	...
	async def connect_to_kodi(self):
		logging.info(f"kodi settings: {self.config['kodi_adress']},  {self.config['kodi_http_port']} {self.config['kodi_ws_port']}, {self.config['kodi_username']}, , {self.config['kodi_password']}")
		self.kodi_connection = get_kodi_connection(self.config['kodi_adress'], self.config['kodi_http_port'], self.config['kodi_ws_port'], self.config['kodi_username'], self.config['kodi_password'], False, 5, None)
	...
	async def connect_to_home(self):
		logging.info(f"kodi settings: {self.config['home_adress']},  {self.config['home_ssl']}, {sys.argv[1]}")
		if self.config['home_ssl']:
			home_ssl = True
		else:
			home_ssl = None
		self.websocket = await websockets.connect(self.config['home_adress'], ssl=home_ssl)
